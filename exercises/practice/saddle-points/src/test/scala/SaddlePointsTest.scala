...
class SaddlePointsTest extends AnyFunSuite with Matchers {
  ...
  test("Can identify saddle points in a non square matrix") {
    pending
    Matrix(List(List(3, 1, 3), List(3, 2, 4))).saddlePoints should be(Set((0, 2), (0, 0)))
  }

  test(
    "Can identify that saddle points in a single column matrix are those with the minimum value") {
    pending
    Matrix(List(List(2), List(1), List(4), List(1))).saddlePoints should be(Set((1, 0), (3, 0)))
  }

  test("Can identify that saddle points in a single row matrix are those with the maximum value") {
    pending
    Matrix(List(List(2, 5, 3, 5))).saddlePoints should be(Set((0, 1), (0, 3)))
  }
}
