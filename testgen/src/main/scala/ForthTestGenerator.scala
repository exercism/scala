...
import testgen.TestSuiteBuilder.{toString, *}
import testgen.*
  ...
  def main(args: Array[String]): Unit =
    ...
    def fromLabeledTestFromInput(argNames: String*): ToTestCaseData =
      withLabeledTest { sut => labeledTest =>
        ...
        val sutCall =
          if isErr then s"""forth.eval($args).isLeft"""
          else s"""forth.eval($args).fold(_ => "", _.toString)"""
    ...
    val code =
      TestSuiteBuilder.build(file, fromLabeledTestFromInput("instructions"), Seq(), Seq("private val forth = new Forth"))
