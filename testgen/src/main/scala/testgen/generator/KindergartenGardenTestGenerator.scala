  ...
  def main(args: Array[String]): Unit =
    ...
    def fromLabeledTestFromInput(): ToTestCaseData =
      withLabeledTest { sut => labeledTest =>
        ...
        val sutCall                =
          if students.isEmpty then s"""Garden.defaultGarden(${escape(diagram.toString)}).$property("$student")"""
          else s"""Garden(List(${students.map(s => escape(s)).mkString(", ")}), ${escape(diagram.toString)}).$property(${escape(student.toString)})"""
