...
object ClockTestGenerator:
  ...
  private def toOperation(function: String): String =
    if function.equalsIgnoreCase("subtract") then "-" else "+"
  ...
  def fromLabeledTestAltFromInput(propArgs: (String, Seq[String])*): ToTestCaseData =
    withLabeledTest { sut => labeledTest =>
      ...
      val sutCall =
        if sutFunc == "create" then s"$sut($args)"
        else if sutFunc == "add" || sutFunc == "subtract" then
      ...
      val expected =
        if sutFunc == "create" || sutFunc == "add" || sutFunc == "subtract" then s"$sut(${expectedToClockStr(labeledTest.expected)})"
        else toString(labeledTest.expected)
