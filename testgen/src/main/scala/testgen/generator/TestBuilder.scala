...
case class TestCaseGen(description: String, callSUT: String, expected: String, result: String, checkResult: String)

object TestCaseGen:
  def apply(description: String, callSUT: String, expected: String): TestCaseGen =
    TestCaseGen(description, callSUT, expected, s"val result = $callSUT", s"result should be ($expected)")
...
class TestBuilder(testName: String):
  ...
  private lazy val printTestCases: String =
    ...
    def printPending: String =
      if pending then s"pending\n    " else { pending = true; "" }

    def printTestCaseGroup(testCaseGroup: Seq[TestCaseGen], groupDescription: Option[String]): String =
