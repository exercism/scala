...
final case class TestCaseData(description: String, sutCall: String, expected: String, pending: Boolean = true)
...
trait TestGenerator[I: Reads, S: Reads]:
  ...
  // protected def withLabeledTest(f: String => TestItem => TestCaseData): ToTestCaseData =
  //   sut => item => f(sut)(item)

// object TestSuiteBuilder:

//   type TestSuiteTemplate    = Template1[TestSuiteData, Txt]
  // type ToTestCaseData[I, S] = String => LabeledTestItem[I, S] => TestCaseData
...
//   type ToOptionTestCaseData = String => LabeledTestItem => Option[TestCaseData]
  ...
  // def build(file: File, toTestCaseData: ToTestCaseData, imports: Seq[String] = Seq(), statements: Seq[String] = Seq())(implicit
  //   template: TestSuiteTemplate = DefaultTemplate,
  // ): String =
  //   val exercise @ Exercise(name, version, cases, comments) =
  //     CanonicalDataParser.parse(file)
  //   val sutName = sutNameOf(name)
  //   val tsName                                              = sutName + "Test"
  //   val testCasesAllPending                                 = cases.map(toTestCaseData(sutName))
  //   val testCases                                           =
  ...
  // def buildFromList(file: File, toTestCaseData: ToTestCaseDataList, imports: Seq[String] = Seq(), statements: Seq[String] = Seq())(implicit
  //   template: TestSuiteTemplate = DefaultTemplate,
  // ): String =
  //   val exercise @ Exercise(name, version, cases, comments) =
  //     CanonicalDataParser.parse(file)
  ...
  // def buildFromOption(file: File, toTestCaseData: ToOptionTestCaseData, imports: Seq[String] = Seq())(implicit
  //   template: TestSuiteTemplate = DefaultTemplate,
  // ): String =
  //   val exercise @ Exercise(name, version, cases, comments) =
  //     CanonicalDataParser.parse(file)
  ...
  //     testCasesAllPending updated (0, testCasesAllPending.head.copy(pending = false))
  //   val testSuiteData                                       =
  //     TestSuiteData(tsName, version.getOrElse(""), imports, testCases)
  ...
  //     Map("hello-world" -> fromLabeledTest(), "sum-of-multiples" -> fromLabeledTest("factors", "limit"), "bowling" -> fromLabeledTest("previous_rolls"))
  ...
  def sutNameOf(exerciseName: String): String =
  ...
  //   if "\"\n" exists (str.contains(_: Char)) then "\"\"\"" else "\""
...
// case class TestCaseData(description: String, sutCall: String, expected: String, pending: Boolean = true)

// case class TestSuiteData(name: String, version: String, imports: Seq[String], testCases: Seq[TestCaseData], statements: Seq[String] = Seq())
