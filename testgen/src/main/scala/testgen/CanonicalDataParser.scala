...
object CanonicalDataParser:
  ...
  private def restoreInts(any: Any): Any =
    any match
      ...
      case map: Map[?, ?]                                      => map mapValues restoreInts
      case seq: Seq[?]                                         => seq map restoreInts
      case any                                                 => any
...
object Exercise:
  implicit def fromParseResult(result: ParseResult): Exercise =
    ...
    Exercise(getRequired(result, "exercise"), getRequired(result, "version"), flattenCases(cases, List()), getOptional(result, "comments"))
...
object LabeledTestItem:
  implicit def fromParseResult(result: ParseResult): LabeledTestItem =
    if result.contains("cases") then result: LabeledTestGroup
    else result: LabeledTest

case class LabeledTest(description: Description, property: Property, expected: Expected, result: ParseResult, parentDescriptions: List[String] = List())
...
object LabeledTest:
  implicit def fromParseResult(result: ParseResult): LabeledTest =
    ...
    LabeledTest(getRequired(result, "description"), getRequired(result, "property"), expected, result)
